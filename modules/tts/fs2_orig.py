...
import numpy as np
from tasks.tts.GRL import EmotionClassifier
...
class FastSpeech2Orig(FastSpeech):
    def __init__(self, dict_size, hparams, out_dims=None):
        ...
        if hparams['use_emo_lookup']:
            self.emo_id_proj = Embedding(hparams['num_emo'], self.hidden_size // 2)
        self.emo_VAD_inten_proj = nn.Linear(1, self.hidden_size // 2, bias=True)
        self.emo_VAD_style_proj = nn.Linear(2, self.hidden_size // 2, bias=True)
        self.emo_proj = nn.Linear(self.hidden_size // 2, self.hidden_size // 2, bias=True)
        self.emo_cls_sty = EmotionClassifier(emo_num = hparams['num_emo'], embed_dim = self.hidden_size // 2, lambda_reversal = hparams['sty_lambda_reversal'])
    ...
    def forward(self, txt_tokens, mels=None, mel2ph=None, spk_embed=None, spk_id=None, emo_id=None, f0=None, uv=None, energy=None, emo_VAD_style=None, emo_VAD_inten=None, infer=False, **kwargs):
        ...
        style_embed = 0
        emo_style = self.emo_VAD_style_proj(emo_VAD_style)
        ret['VAD_style_final'] = self.emo_cls_sty(emo_style)
        style_embed = style_embed + emo_style
        
        # Softplus
        combined_embedding = emos_embed + style_embed
        emotion_embedding = F.softplus(combined_embedding)
        ret['emo_embed'] = emo_all_emb = (intens_embed + emotion_embedding)[:, None, :]
