...
class FastSpeechTask(SpeechBaseTask):
    ...
    def run_model(self, sample, infer=False, *args, **kwargs):
        ...
        if not infer:
            ...
            output = self.model(txt_tokens, mels=target,mel2ph=mel2ph, spk_id=spk_id, emo_id=emo_id, f0=f0, uv=uv, infer=False)
            losses = {}
            ...
            if use_gt_f0:
                f0 = sample['f0']
                uv = sample['uv']
            output = self.model(txt_tokens, mels=target, mel2ph=mel2ph, spk_id=spk_id, emo_id=emo_id, f0=f0, uv=uv, infer=True)
            return output
    ...
    def save_valid_result(self, sample, batch_idx, model_out):
        ...
        if self.global_step > 0:
            ...
            self.logger.add_audio(f'wav_val_{batch_idx}_{emo}', wav_pred, self.global_step, sr)
            # with gt duration
            ...
            self.logger.add_audio(f'wav_gdur_{batch_idx}_{emo}', wav_pred, self.global_step, sr)
            self.plot_mel(batch_idx, sample['mels'], model_out['mel_out'][0], f'mel_gdur_{batch_idx}_{emo}',
            ...
            if not hparams['use_gt_dur']:
                ...
                self.plot_mel(batch_idx, sample['mels'], model_out['mel_out'][0], f'mel_pdur_{batch_idx}_{emo}',
                              dur_info=dur_info, f0s=f0_gt)
                wav_pred = self.vocoder.spec2wav(model_out['mel_out'][0].cpu(), f0=f0_gt)
                self.logger.add_audio(f'wav_pdur_{batch_idx}_{emo}', wav_pred, self.global_step, sr)
        # gt wav
    ...
    def test_step(self, sample, batch_idx):
        ...
        self.saving_result_pool.add_job(self.save_result, args=[
            wav_pred, mel_pred, base_fn, gen_dir, str_phs, mel2ph_pred])
        if hparams['save_gt']:
            wav_gt = self.vocoder.spec2wav(mel_gt)
            # self.saving_result_pool.add_job(self.save_result, args=[
            #     wav_gt, mel_gt, base_fn % 'G', gen_dir, str_phs, mel2ph])
            self.saving_result_pool.add_job(self.save_result, args=[
                wav_gt, mel_gt, base_fn, gen_dir, str_phs, mel2ph])
        return {
            'item_name': item_name,
            'text': text,
