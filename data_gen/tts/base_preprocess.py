...
class BasePreprocessor:
    def __init__(self):
        ...
        self.emo_map_fn = f"{self.processed_dir}/emo_map.json"
    ...
    def process(self):
        ...
        for item_, (item_id, item) in zip(meta_data, multiprocess_run_tqdm(process_item, args, desc='Preprocess')):
            if item is not None:
                ...
                item['emo_name'] = item.get('emo_name', '<SINGLE_EMO>')
                item['others'] = item.get('others', None)
                phone_list += item['ph'].split(" ")
                word_list += item['word'].split(" ")
                spk_names.add(item['spk_name'])
                emo_names.add(item['emo_name'])
                items.append(item)

        # add encoded tokens
        ph_encoder, word_encoder = self._phone_encoder(phone_list), self._word_encoder(word_list)
        spk_map = self.build_spk_map(spk_names)
        emo_map = self.build_emo_map(emo_names)
        args = [{
            'ph': item['ph'], 'word': item['word'], 'spk_name': item['spk_name'], 'emo_name': item['emo_name'],
            'word_encoder': word_encoder, 'ph_encoder': ph_encoder, 'spk_map': spk_map, 'emo_map': emo_map
        } for item in items]
    ...
    @classmethod
    def preprocess_second_pass(cls, word, ph, spk_name, emo_name,word_encoder, ph_encoder, spk_map, emo_map):
        word_token = word_encoder.encode(word)
        ph_token = ph_encoder.encode(ph)
        spk_id = spk_map[spk_name]
        emo_id = emo_map[emo_name]
        return {'word_token': word_token, 'ph_token': ph_token, 'spk_id': spk_id, 'emo_id': emo_id}
    ...
    def build_emo_map(self, emo_names):
        emo_map = {x: i for i, x in enumerate(sorted(list(emo_names)))}
        assert len(emo_map) == 0 or len(emo_map) <= hparams['num_emo'], len(emo_map)
        print(f"| Number of emos: {len(emo_map)}, emo_map: {emo_map}")
        json.dump(emo_map, open(self.emo_map_fn, 'w'), ensure_ascii=False)
        return emo_map
    ...
    def load_emo_map(self, base_dir):
        emo_map_fn = f"{base_dir}/emo_map.json"
        emo_map = json.load(open(emo_map_fn, 'r'))
        return emo_map
